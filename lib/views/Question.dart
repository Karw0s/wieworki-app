...
class QuestionScreen extends StatelessWidget {
  ...
  const QuestionScreen({Key key, this.color}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    String jQuestion;
    final loadString = DefaultAssetBundle.of(context)
        .loadString("assets/questions.json");
    print(loadString);
    final q = loadString.then((value) => jQuestion = jsonDecode(value));
    //     .asStream()
    //     .map((event) => Question.fromJson(event))
    //     .toList();
    //
    print(q);
    print(jQuestion);
